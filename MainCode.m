...
% Lakdawala (2015) Proxy-SVAR-IV Identification (Partial Identification)
%% *************************************************************************************************************
clc; clear; close all;
addpath functions;

% Global variables - Essential for the MinimumDistance function
global NLags TAll sig_CMD mS_opt n r g_shock M Sigma_u mDDr

%% 1. SETTAGGIO SPECIFICA
...
% SPECIFICA FISSA: [FF, GS1, LCPI, LIP] = [FFR, 1Y, log(CPI), log(IP)]

var_idx   = [3 4 1 2];
var_names = {'FFR','1Y','log(CPI)','log(IP)'};

% Dataset VAR (full sample, il trimming lo fai dopo con start/end date)
DataSet = data_matrix(:, var_idx);

% Check rapido: stampa le labels selezionate (per evitare errori stupidi)
fprintf('\nSelected series:\n');
for j = 1:numel(var_idx)
    fprintf('  %s  <- col %2d : %s\n', var_names{j}, var_idx(j), all_labels(var_idx(j)));
end
% --- Costruisci dataset VAR ---
DataSet = data_matrix(:, var_idx);

% --- Dimensioni (Fanelli cap.5) ---
n = size(DataSet,2);    % più robusto di numel(var_idx)
r = 2;                  % TP1 => 2 proxies
g_shock = 2;            % Target + Path
M = n + r;              % dimensione stackata (u,z)
...
%% 3. FILTRO DATE E CAMPIONE (Lakdawala-style: exact anchors)
...
% TAll for Fanelli machinery (effective sample after lags)
TAll = size(AllDataSet,1) - NLags;
assert(TAll > 0, 'TAll <= 0: check NLags or sample length.');
%% 4. LOAD INSTRUMENTS (TP1) + ALIGN END OF SAMPLE (Lakdawala-style)
...
% --- Make U and rf_dates consistent (handles both possible U_full conventions) ---
if size(U_full,1) == T
...
%% PARTIAL IDENTIFICATION (Angelini-Fanelli style) using (u,z) moments only

Tm = size(U_m,1);
W  = [U_m, Z_m];                 % Tm x M
Sigma_Eta_Sample = (W' * W) / Tm; % M x M   (this is their Sigma_Eta_Sample)
...
%% --- Useful matrices (general, no hard-code) ---
...
% Omega_eta = 2 * D^+ * (Sigma ⊗ Sigma) * (D^+)'   (same as AF)
Omega_eta = 2 * mDD * kron(Sigma_Eta_Sample, Sigma_Eta_Sample) * mDD';

%% --- Build P_matrix automatically (AF hard-coded it for M=5; here we generalize) ---
...
%% --- Map lambda -> zeta moments (AF Supplement, same structure) ---
% zeta stacks:
...
B10(1, 2) = 0; % Assicurati che il punto di partenza rispetti il vincolo
theta0 = [B10(:); log(Phi0(1,1)); Phi0(2,1); log(Phi0(2,2))];
...
%% --- CHECK IDENTIFICABILITÀ (Jacobian Rank Check) ---
% Definiamo la funzione per calcolare zeta(theta) analiticamente
% theta = [vec(B1); a11; a21; a22]
zeta_model = @(th) func_zeta_theta(th, n, r, g_shock, Sigma_u, mDDr);

% Calcolo della Jacobiana numerica nel punto di ottimo theta_hat
% Nota: Puoi usare 'jacobianest' se hai il toolbox di Derivest, 
% altrimenti usiamo la funzione integrata di MATLAB
...
fprintf('\n--- DIAGNOSTICA IDENTIFICAZIONE ---\n');
fprintf('Numero di parametri (k): %d\n', num_params);
fprintf('Numero di momenti (m):   %d\n', num_moments);
fprintf('Rango della Jacobiana:   %d\n', rank_J);
fprintf('Condition Number:        %.2e\n', cond_J);

if rank_J < num_params
    warning('ATTENZIONE: La Jacobiana NON è a pieno rango (Rank < k). Il modello è localmente non identificato.');
else
    fprintf('Successo: La Jacobiana è a pieno rango. Requisito di identificazione locale soddisfatto.\n');
end
fprintf('------------------------------------\n');
...
Hh = 48;                     % horizon in months (e.g. 48 = 4 years)
p  = NLags;
...
%% REPLICA TABELLA 1 (LAKDAWALA, 2019) - SOLUZIONE FINALE
% Regressione per FFR residual
mdl_ffr = fitlm(Z_m, U_m(:,1)); 
% Regressione per 1Y residual (Colonna a del paper)
mdl_1y  = fitlm(Z_m, U_m(:,2)); 

fprintf('\n--- REPLICA TABELLA 1 (Campione: %d oss) ---\n', mdl_1y.NumObservations);

% Estrazione F-statistic e p-value in modo universale
stats_1y = anova(mdl_1y,'component',1);
f_stat_1y = mdl_1y.ModelCriterion.AIC; % AIC/BIC sono pronti, ma per la F usiamo:
f_val_1y = mdl_1y.ModelFitVsNullModel.Fstat;

fprintf('\nRISULTATI PER 1 YEAR RESIDUAL (Atteso R-squared: 0.101)\n');
...
fprintf('\nCOEFFICIENTI STIMATI (1Y Residual):\n');
% Controlla se x2 (Path Factor) e vicino a 0.347
disp(mdl_1y.Coefficients);

fprintf('\nRISULTATI PER FFR RESIDUAL (Atteso R-squared: 0.111)\n');
...
%% VISUALIZZAZIONE PROFESSIONALE DEGLI STRUMENTI (PROXY)
% Verifica che r sia definito, altrimenti lo ricava
...
proxy_colors = {[0.2, 0.4, 0.8], [0.8, 0.4, 0.2]}; % Blu per Target, Arancio per Path

for i = 1:r
    nexttile;
    
    % Plot dell'area con trasparenza per visualizzare la densità degli shock
    fill_color = proxy_colors{i};
    area(dates_m, Z_m(:,i), 'FaceColor', fill_color, 'EdgeColor', fill_color, ...
         'FaceAlpha', 0.3, 'LineWidth', 1.5);
    
    hold on;
    % Linea dello zero solida
    ...
    % Formattazione asse X con le date
    datetick('x', 'yyyy', 'keeplimits');
    ylabel('Units');
    
    % Limiti asse Y simmetrici per chiarezza
...
%% BOOTSTRAP CON RI-STIMA DEL VAR (Full Uncertainty)
fprintf('\nInizio Bootstrap con ri-stima VAR (%d iterazioni)...\n', BootstrapIterations);
IRF_boot = zeros(n, g_shock, Hh+1, BootstrapIterations);
IRF_Point = IRF; 

% Estrai i coefficienti del VAR originale per la ricostruzione dei dati
Ahat_orig = EstMdl.AR; % Cell array p x 1
Const_orig = EstMdl.Constant;

for boot = 1:BootstrapIterations
    % --- 1. GENERAZIONE DATI SINTETICI (Residual-based Bootstrap) ---
    Tm_curr = size(U_m, 1);
    % Wild Bootstrap sui residui matched (per la parte strutturale)
    eta = (randn(Tm_curr, 1) > 0) * 2 - 1; 
    U_b_matched = U_m .* repmat(eta, 1, n);
    Z_b = Z_m .* repmat(eta, 1, r);
    
    % Per la ri-stima del VAR, abbiamo bisogno di residui per tutto il campione T_eff
    % Usiamo il Wild Bootstrap sull'intero set di residui U
    T_total = size(U, 1);
    eta_total = (randn(T_total, 1) > 0) * 2 - 1;
    U_b_full = U .* repmat(eta_total, 1, n);
    
    % Ricostruiamo la serie storica DataSet_b partendo dai coefficienti stimati
    DataSet_b = zeros(size(AllDataSet));
    DataSet_b(1:NLags, :) = AllDataSet(1:NLags, :); % Condizioni iniziali fisse
    ...
    % --- 2. RI-STIMA DEL VAR SUI DATI SINTETICI ---
    Mdl_b = varm(n, NLags);
    Mdl_b.Constant = NaN(n,1);
    [EstMdl_b, ~, ~, ~] = estimate(Mdl_b, DataSet_b, 'Display', 'off');
    Ahat_b = EstMdl_b.AR;

    % --- 3. RICALCOLO MOMENTI E mS_opt (Fanelli) ---
    W_b = [U_b_matched, Z_b];
    ...
    if exitflag_b > 0
        ...
        % IRF strutturale per questa iterazione
        for h = 0:Hh
...
%% 13. PLOT FINALI CON BANDE SUP-T (UNIFICATI) - PROFESSIONAL STYLE
fprintf('\nGenerazione grafici unificati...\n');
alpha_coverage = 10; 
hgrid = 0:Hh;

% Creiamo una UNICA figura grande per entrambi gli shock
figure('Name', 'Structural IRFs: Target vs Path Shock', 'Color', 'w', ...
       'Units', 'normalized', 'Position', [0.1, 0.1, 0.7, 0.85]);

% Layout: n righe (variabili) e g_shock colonne (shock)
t = tiledlayout(n, g_shock, 'TileSpacing', 'compact', 'Padding', 'compact');

for j = 1:n % Ciclo esterno sulle variabili (righe)
    for k = 1:g_shock % Ciclo interno sugli shock (colonne)
        ...
        % Calcolo Bande simultanee (Algorithm 3)
