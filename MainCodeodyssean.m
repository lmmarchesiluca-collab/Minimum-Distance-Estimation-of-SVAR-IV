...
% Global variables per le funzioni di ottimizzazione
global NLags TAll sig_CMD mS_opt n r g_shock M Sigma_u mDDr

%% 1. SETTAGGIO E DATI
NLags = 12;
target_size = 1; % Normalizzazione a 1 unità (es. 100bp o 1%)
path_size   = 1;
...
%% 2. FILTRO DATE E STIMA VAR RIDOTTO
...
% Gestione residui e date effettive
...
%% 3. CARICAMENTO E ALLINEAMENTO STRUMENTI
% --- MODELLO A: ORIGINALE (TP1_instr.mat) ---
...
% --- MODELLO B: ODYSSIAN (TP1_pvt_res_instr.mat) ---
...
fprintf('Allineamento completato: %d osservazioni per Original, %d per Odyssian.\n', size(Z_m1,1), size(Z_m2,1));
%% 4. MATRICI DI SUPPORTO E VALORI INIZIALI
...
% Calcolo theta0 (Starting Values) usando il primo set
...
B10(1,2) = 0; % Restrizione Target/Path
theta0 = [B10(:); log(Phi0(1,1)); Phi0(2,1); log(Phi0(2,2))];

%% 5. IDENTIFICAZIONE E CALCOLO IRF
Hh = 48;
...
%% 6. PLOT DI CONFRONTO (PATH SHOCK)
hgrid = 0:Hh;
...
function IRF = run_full_identification(U_m, Z_m, Psi, n, r, g_shock, options, theta0, P_matrix, mDD, mDDn, mDDr)
    ...
    % Normalizzazione: 1 unità su FFR (shk1) e 1Y (shk2)
